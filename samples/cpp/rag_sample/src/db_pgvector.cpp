...
void DBPgvector::db_setup() {
    pqxx::work tx{pgconn};
    tx.exec0("CREATE EXTENSION IF NOT EXISTS vector");
    tx.exec0("DROP TABLE IF EXISTS documents");
    // max vector dim is 16000
    tx.exec0("CREATE TABLE documents (id bigserial PRIMARY KEY, content text, embedding vector(1024))");
    tx.commit();
...
std::vector<std::string> DBPgvector::db_retrieval(size_t chunk_num,
    ...
    try {
        ...
        pqxx::result res{
            tx.exec_params("SELECT id, content, embedding FROM documents WHERE id != $1 ORDER BY embedding <=> $2 LIMIT 3",
                           chunk_num + 1,
                           embeddings_vector)};
