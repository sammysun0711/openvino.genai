...
def load_image_from_url(link, target_size=None):
    """
    ...
    try:
        ...
        response = requests.get(link, stream=True, timeout=10) # Added timeout

        # Check if the request was successful (status code 200)
        if response.status_code == 200:
            # Ensure the raw stream is decoded before passing to Image.open
            # response.raw.decode_content = True # This can sometimes help

            # More robust way: read the content and use io.BytesIO
            image_bytes = response.content
            
            # Check if we actually got some bytes
            if not image_bytes:
                print(f"Error: No content received from URL: {link}")
                return None

            try:
                # Open the image from the bytes using io.BytesIO
                image = Image.open(io.BytesIO(image_bytes))
                print(f"Image loaded successfully from {link}")
                ...
                print(f"Error: Cannot identify image file from URL: {link}.")
                print("This could be due to an invalid image format or corrupted data.")
                # You could also inspect response.headers['Content-Type'] here
            ...
            print(f"Error: Failed to retrieve image from {link}. Status code: {response.status_code}")
            return None
    except requests.exceptions.RequestException as e:
        print(f"Error: Request failed for URL {link}: {e}")
        return None
    except Exception as e:
...
def get_pil_image_by_path(path, target_size=None):
    """
    Get PIL image by link.
    Args:
        link (str): Link to the image.
...
def get_image_by_path(path):
    ...
    image = get_pil_image_by_path(path)
    image_data = np.array(image)
    return ov.Tensor(image_data)
...
configs = [get_greedy()]
def test_vlm_continuous_batching_vs_stateful(models_path, device, config):
    scheduler_config = SchedulerConfig()
    #models_path = get_ov_model(model_ids[0])
    cb_pipe = ContinuousBatchingPipeline(
        models_path,
    ...
    #print("image_links_list: ", image_links_list)
    #image_dir = 'images'
    #import glob
    #image_list = sorted(glob.glob(r"images/*.*"))
    ...
    for links in image_links_list:
        ...
        for link in links:
            #images.append(get_image_by_link(link))
            images.append(load_image_from_url(link))
    ...
    for idx, links in enumerate(image_links_list):
        ...
        res_stateful_list.append(res_stateful)

    cb_perf_metrics = res_cb[1][0].perf_metrics
    stateful_perf_metrics = res_stateful_list[0].perf_metrics

    print("perf_metrics is not None:", cb_perf_metrics is not None)

    print(" cb_perf_metrics.get_load_time() ",  cb_perf_metrics.get_load_time())
    num_tokens = cb_perf_metrics.get_num_generated_tokens()
    print(" num generated tokens:",  num_tokens)
    print(" cb_perf_metrics.get_num_input_tokens())",
           cb_perf_metrics.get_num_input_tokens())
    print(" cb_perf_metrics.get_ttft().mean:",
           cb_perf_metrics.get_ttft().mean)
    print(" cb_perf_metrics.get_tpot().mean",
           cb_perf_metrics.get_tpot().mean)
    print(" cb_perf_metrics.get_ipot().mean",
           cb_perf_metrics.get_ipot().mean)
 
    print(" cb_perf_metrics.get_inference_duration().mean:",
           cb_perf_metrics.get_inference_duration().mean)
    print(" cb_perf_metrics.get_generate_duration().mean:",
           cb_perf_metrics.get_generate_duration().mean)
    print(" cb_perf_metrics.get_tokenization_duration().mean:",
           cb_perf_metrics.get_tokenization_duration().mean)
    print(" cb_perf_metrics.get_detokenization_duration().mean:",
           cb_perf_metrics.get_detokenization_duration().mean)
    print(" cb_perf_metrics.get_prepare_embeddings_duration().mean:",
           cb_perf_metrics.get_prepare_embeddings_duration().mean)
    
    print("*******************************************************************\n")

    print(" stateful_perf_metrics.get_load_time() ",  stateful_perf_metrics.get_load_time())
    num_tokens = stateful_perf_metrics.get_num_generated_tokens()
    print(" num generated tokens:",  num_tokens)
    print(" stateful_perf_metrics.get_num_input_tokens())",
           stateful_perf_metrics.get_num_input_tokens())
    print(" stateful_perf_metrics.get_ttft().mean:",
           stateful_perf_metrics.get_ttft().mean)
    print(" stateful_perf_metrics.get_tpot().mean",
           stateful_perf_metrics.get_tpot().mean)
    print(" stateful_perf_metrics.get_ipot().mean",
           stateful_perf_metrics.get_ipot().mean)
 
    print(" stateful_perf_metrics.get_inference_duration().mean:",
           stateful_perf_metrics.get_inference_duration().mean)
    print(" stateful_perf_metrics.get_generate_duration().mean:",
           stateful_perf_metrics.get_generate_duration().mean)
    print(" stateful_perf_metrics.get_tokenization_duration().mean:",
           stateful_perf_metrics.get_tokenization_duration().mean)
    print(" stateful_perf_metrics.get_detokenization_duration().mean:",
           stateful_perf_metrics.get_detokenization_duration().mean)
    print(" stateful_perf_metrics.get_prepare_embeddings_duration().mean:",
           stateful_perf_metrics.get_prepare_embeddings_duration().mean)
...
#     assert  perf_metrics.get_load_time() < load_time
#     num_tokens = perf_metrics.get_num_generated_tokens()
#     assert  num_tokens <= max_new_tokens
#     assert  perf_metrics.get_num_input_tokens() < len(prompts[0]) + image_tokens_num
#     assert  perf_metrics.get_ttft().mean < generate_time
#     assert  perf_metrics.get_tpot().mean < generate_time / num_tokens
#     assert  perf_metrics.get_ipot().mean < generate_time / num_tokens
#     assert (num_tokens - 1) / (
#         (generate_time - perf_metrics.get_ttft().mean) / 1000.0
#     ) < perf_metrics.get_throughput().mean

#     assert  perf_metrics.get_inference_duration().mean < generate_time
#     assert  perf_metrics.get_generate_duration().mean < generate_time
#     assert  perf_metrics.get_tokenization_duration().mean < generate_time
#     assert  perf_metrics.get_detokenization_duration().mean < generate_time
#     assert  perf_metrics.get_prepare_embeddings_duration().mean < generate_time

#     squared_generate_time = generate_time * generate_time
#     assert  perf_metrics.get_ttft().std < squared_generate_time
#     assert  perf_metrics.get_tpot().std < squared_generate_time
#     assert  perf_metrics.get_ipot().std < squared_generate_time
#     assert  perf_metrics.get_throughput().std < squared_generate_time
#     assert  perf_metrics.get_inference_duration().std < squared_generate_time
#     assert  perf_metrics.get_generate_duration().std < squared_generate_time
#     assert  perf_metrics.get_tokenization_duration().std < squared_generate_time
#     assert  perf_metrics.get_detokenization_duration().std < squared_generate_time
#     assert (
#          perf_metrics.get_prepare_embeddings_duration().std < squared_generate_time
#     )
...
if __name__ == "__main__":
    models_path = "MiniCPM-V-2_6-ov"
    device = "CPU" 
    configs = get_greedy()
    test_vlm_continuous_batching_vs_stateful(models_path, device, configs)