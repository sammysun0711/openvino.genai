...
void apply_kv_cache_precision(const std::shared_ptr<ov::Model>& model,
                              const std::string& device,
                              const ov::AnyMap& plugin_config) {
    ov::element::Type m_kv_cache_type = ov::element::undefined,
                      ir_kv_cache_precision = get_model_kv_cache_precision(model);
    ...
    const bool accuracy_mode =
        execution_mode_it != plugin_config.end() &&
...
void ContinuousBatchingPipeline::ContinuousBatchingImpl::initialize_pipeline(
    ...
    // apply LoRA
    if (auto filtered_properties =
            extract_adapters_from_properties(mutable_properties, &m_generation_config.adapters)) {
    ...
    if (is_use_cache_eviction) {
        ...
        if (eviction_config.apply_rotation) {
            ...
            size_t max_sequence_cache_occupation_length_in_blocks =
                normalized_config.max_num_batched_tokens / m_block_size + 1;
        ...
        m_model_runner = std::make_shared<ModelRunner>(infer_request, m_block_size, m_num_decoder_layers);
    }
...
std::vector<EncodedGenerationResult> ContinuousBatchingPipeline::ContinuousBatchingImpl::generate(
    ...
    auto all_requests =
        m_awaiting_requests;  // we need to store all requests to get results from them once generation has finished
    ...
    for (auto& seq_id_and_attention_scores : sequence_attention_scores) {
        ...
        if (m_seq_group_id_to_cache_eviction_algo_map.find(seq_id) == m_seq_group_id_to_cache_eviction_algo_map.end()) {
            m_seq_group_id_to_cache_eviction_algo_map[seq_id] =
                CacheEvictionAlgorithm(sched_config.cache_eviction_config, m_block_size, num_decoder_layers);
        }
